...
在基本操作教程当中，我们只会对防火墙的**默认区域**进行操作，而不会去接触其他区域或者接口。
...
```
*在接下来可能会多次引用这个示例，所以在这里起一个莫名其妙的名字：虚伪的结果*
...
虚伪的结果的第二行，`target: default`，这告诉我们这个区域当前的目标是`default`，
注意这**不是**意味着这个区域是默认区域，区域的目标 与 防火墙默认区域 没有关系。
...
可以定义一系列服务，并直接添加到防火墙区域，以允许外部访问对应的服务。
...
在上方的管理过程中，我们一直都在对**默认区域**进行操作。但是，防火墙当中可以配置多个区域。

## 来源管理

来源一般指IP地址，可以将指定IP地址切换到不同的区域。

## 端口转发

防火墙可以配置简单的端口转发。`--add-forward`、`--add-forward-port`
